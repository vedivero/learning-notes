...
public class EmployeeApiController {
    ...
    public ResponseEntity<?> getAllEmployees() {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("직원 목록 조회 중 오류가 발생했습니다. : " + e.getMessage());
        }
    }
    ...
    public ResponseEntity<?> getEmployeeDetail(@PathVariable("employeeId") int employeeId) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("직원 상세 정보 조회 중 오류가 발생했습니다. : " + e.getMessage());
        }
    }
    ...
    public ResponseEntity<?> searchEmployees(
        ...
        try {
            Page<EmployeeListResponse> response = employeeService.searchEmployees(keyword, pageable);
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("직원 검색 중 오류가 발생했습니다. : " + e.getMessage());
        }
    }
