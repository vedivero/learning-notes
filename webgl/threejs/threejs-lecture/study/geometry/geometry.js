...
class App {
	...
	_setupModel() {
		...
		const fillMaterial = new THREE.MeshPhongMaterial({ color: 0x515151 }); // 회색 계열의 정육면체 생성을 위해 Mesh 타입의 Object 생성 코드
		const cube = new THREE.Mesh(geometry, fillMaterial); // geometry객체와 material 객체를 통해 Mesh가 생성
