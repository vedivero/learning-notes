...
class App {
	...
	_setupModel() {
		// 이 재질은 mesh를 지정된 색상으로 렌더링
		const material = new THREE.MeshBasicMaterial({
			// 부모 클래스인 Material의 속성
			visible: true, // 랜더링 시, mesh 표출 여부
			transparent: false, // 재질에 대한 불투명도 속성인 opcity 사용 여부
			opacity: 1, // 0 : 완전 투명, 1 : 완정 불투명
			depthTest: true, // 렌더링되고 있는 Mesh의 픽셀 위치의 z값을, 버퍼 값을 이용해 검사할지에 대한 여부
			depthWrite: true, // 랜더링되고 있는 Mesh의 픽셀에 대한 z값을 깊이 버퍼에 기록할지에 대한 여부
			side: THREE.FrontSide, // Mesh를 구성하는 삼각형 면에 대해, 앞 면만 렌더링할 것인지, 뒷 면만 렌더링 할 것인지, 모두 렌더링 할 것인지를 지정
