...
class App {
	...
	_setupModel() {
		// 이 재질은 mesh를 구성하는 정점에서 광원의 영향을 계산하는 재질
		const material = new THREE.MeshLambertMaterial({
			// 부모 클래스인 Material의 속성
			transparent: false, // 재질에 대한 불투명도 속성인 opcity 사용 여부
			opacity: 1, // 0 : 완전 투명, 1 : 완정 불투명
			side: THREE.DoubleSide, // Mesh를 구성하는 삼각형 면에 대해, 앞 면만 렌더링할 것인지, 뒷 면만 렌더링 할 것인지, 모두 렌더링 할 것인지를 지정
			// FrontSide | BackSide | DoubleSide
			color: 0xd25383,
			emissive: 0x00000, // 다른 광원에 영향을 받지 않는 재질 자체에서 방출하는 색상 값, 기본값:black, 방출하지
