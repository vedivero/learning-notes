package vedivero.board.common.snowflake;
...
class SnowflakeTest {
	...
	void nextIdTest() throws ExecutionException, InterruptedException {
		...
		// 10개의 쓰레디 풀이 1,000번 동안, 1,000개의 ID를 생성
		// when
		...
		for (Future<List<Long>> future : futures) {
			...
			for (int i = 1; i < idList.size(); i++) {
				assertThat(idList.get(i)).isGreaterThan(idList.get(i - 1)); // 오름차순
			}
			result.addAll(idList);
		}
		// 중복 없이 count => 100만개 ID 생성되는지 테스트
	...
	void nextIdPerformanceTest() throws InterruptedException {
		...
		// snowflake가 빠르게 ID를 생성 = 100만개 264ms
